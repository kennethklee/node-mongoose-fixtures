...
var fixtures = module.exports = function(dataset, db, callback) {
    ...
    if (typeof dataset !== 'object') {
        if (callback) return callback.call(fixtures, new Error('Dataset not a valid object or does not exist as a named fixture.'));
    }
...
fixtures.save = function(name, data, callback) {
    ...
    if (callback) callback.call(null, null, oldFixture);
};
...
fixtures.clear = function(name, callback) {
    if (typeof name === 'function') callback = name, name = null;
...
fixtures.reset = function(modelName, db, callback) {
    if (modelName instanceof mongoose.Mongoose) callback = db, db = modelName, modelName = null;
    if (typeof db === 'function') callback = db, db = mongoose;
